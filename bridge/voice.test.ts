...
function relay(overrides: Partial<VoiceSocketData> = {}) {
...
async function waitUntil(condition: () => boolean): Promise<void> {
  ...
  while (!condition()) {
    if (Date.now() >= deadline) throw new Error("timed out waiting for asynchronous Voice event");
    await Bun.sleep(1);
  }
}
...
describe("VoiceBroker remote lease", () => {
  ...
  test("keeps STT-only relays out of the OMP handoff", async () => {
    const nativeFetch = globalThis.fetch;
    const nativeWebSocket = globalThis.WebSocket;
    const tokenFile = join(tmpdir(), `collie-voice-${crypto.randomUUID()}.token`);
    const remoteEvents: Array<{ kind: string }> = [];
    ...
    try {
      const browser = relay({
        sessionFile: `\0collie:voice:${JSON.stringify(["session", "pane"])}`,
      ...
      expect(browser.messages).toContain(JSON.stringify({ kind: "handoff-ready", generation: 1, accepted: false }));
      await broker.message(browser as never, JSON.stringify({ kind: "release" }));
