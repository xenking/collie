...
export class TurnController {
  ...
  readonly replySpeechSupported: boolean;
  ...
  constructor(
    ...
    this.replySpeechSupported = ws.data.replySpeechSupported;
  }
  ...
  #state(caption?: { role: "user" | "assistant"; text: string; provisional: boolean }): void {
    const message: Record<string, unknown> = { kind: "voice-state", generation: this.#generation, phase: this.#phase };
...
/**
 * Owns one browser relay per server-resolved session identity. OMP relays use session files;
 * STT-only panes use a server-generated synthetic identity.
 */
export class VoiceBroker {
  ...
  async open(ws: VoiceSocket): Promise<void> {
    ...
    if (ws.data.replySpeechSupported) {
      try {
    ...
    switch (kind) {
      ...
      case "handoff":
        send(ws, {
          kind: "handoff-ready",
          generation,
          accepted: ws.data.replySpeechSupported && relay.handoff(generation),
        });
        return;
      case "resume":
        send(ws, { kind: "resumed", accepted: ws.data.replySpeechSupported && relay.resume() });
        return;
      case "playback-ended":
  ...
  #releaseRelay(session: string, relay: TurnController): void {
    ...
    if (relay.replySpeechSupported) void this.#setRemote(session, "remote-release").catch(() => undefined);
    relay.close();
    this.#relays.delete(session);
  }
