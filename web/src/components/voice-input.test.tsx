...
describe("VoiceInput", () => {
  ...
  it("applies STT-only finals without waiting for an OMP handoff", async () => {
    const onTranscript = vi.fn(async () => false);
    const onVoiceStateChange = vi.fn();
    const onError = vi.fn();
    render(
      <VoiceInput
    ...
    await waitFor(() => expect(onTranscript).toHaveBeenCalledWith("Привет"));
    expect(FakeSocket.instances[0]?.send).not.toHaveBeenCalledWith('{"kind":"handoff","generation":1}');
  ...
  it("keeps the tap-to-toggle control mounted when a provisional caption fills the composer", async () => {
    ...
    expect(screen.getByRole("button", { name: "Stop voice input" })).toBe(button);
